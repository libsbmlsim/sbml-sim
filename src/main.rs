...
fn main() {
    ...
    let mut root = Model::new();
    let mut current = Refs::Model(&mut root);
        ...
        match e {
            ...
            Ok(XmlEvent::StartElement {
                ...
                match name.local_name.as_str() {
                    "listOfSpecies" => match current {
                        Refs::Model(ref mut ptr) => {
                            ptr.list_of_species = Some(ListOfSpecies::new().with_parent(current));
                            current = Refs::ListOfSpecies(&mut ptr.list_of_species.unwrap());
                        }
                        _ => {}
                    },
                    _ => {}
